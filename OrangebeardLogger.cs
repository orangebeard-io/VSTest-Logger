...
namespace Orangebeard.VSTest.TestLogger
{
    ...
    public class OrangebeardLogger : ITestLoggerWithParameters
    {
            ...
            if (_launchReporter != null)
            {
                try
                {
                    ...
                    if (innerResultsCountProperty == null || (innerResultsCountProperty != null && (int)e.Result.GetPropertyValue(innerResultsCountProperty) == 0))
                    {
                        ...
                        string testDescription = FindDescription(e);

                        List<string> testCategories = FindCategories(e);
                        ...
                        AddLogMessages(e, testReporter);

                        AddAttachments(e, testReporter);

                        FinishTest(e, testReporter);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void AddAttachments(TestResultEventArgs e, ITestReporter testReporter)
        {
            // add attachments
        ...
        private void AddLogMessages(TestResultEventArgs e, ITestReporter testReporter)
        {
            // add log messages
        ...
        private static List<string> FindCategories(TestResultEventArgs e)
        {
            // find categories
        ...
        private static string FindDescription(TestResultEventArgs e)
        {
            // find description
